...
Check TM reference format. Valid formats:
  - TM.SUBSYSTEM.mnemonic  (3-part, new)
  - TM1.SUBSYSTEM.mnemonic (3-part with bank number)
  - TM1.mnemonic           (2-part, legacy)
Mnemonics may contain word chars plus +, -, .
"""
function validate_tm_references!(proc::ParsedProcedure, errors::Vector{ValidationError})
    # Valid TM reference patterns
    tm_valid_3part = r"\bTM\d*\.\w+\.[\w+\-.]+"
    tm_valid_2part = r"\bTM\d+\.\w+"

    for line in proc.lines
        raw = line.raw_text
        # Find all TM-like tokens (TM followed by non-space chars)
        for m in eachmatch(r"\bTM[\w+\-.]+\.[\w+\-.]+(?:\.[\w+\-.]+)?", raw)
            token = m.match
            # Strip trailing operators/punctuation that aren't part of mnemonic
            token = replace(token, r"[=<>!&|,\)\s]+$" => "")
            if !occursin(tm_valid_3part, token) && !occursin(tm_valid_2part, token)
                push!(errors, ValidationError(
                    proc.source_file,
                    line.line_number,
                    line.raw_text,
                    "Malformed TM reference: $token",
                    "Use format: TM.SUBSYSTEM.mnemonic or TM<n>.mnemonic",
                    :warning
                ))
            end
        end
