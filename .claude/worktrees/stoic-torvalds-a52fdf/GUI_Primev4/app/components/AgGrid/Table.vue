<template>
  <div id="jana" :style="dynamicStyles">
    <div v-if="props.show_filter" class="ma-2 flex grow">
      ...
      <Button v-if="is_save_allowed" class="w-3/4 ml-2" label="Save" icon="i-mdi:content-save" @click="saveData"
        autofocus />
    </div>
    <div v-if="show_banner">
      <Message :severity="banner_severity">{{ banner_msg }}</Message>
    </div>
    <ag-grid-vue ref="ag_grid" :key="counter" style="width: 100%; height: 100%" :tooltipShowDelay="0"
      :tooltipHideDelay="2000" :defaultColDef="defaultColDef" :columnDefs="ColDefs" :rowData="rowData"
      :rowNumbers="false" :theme="colorModeStore.currentMode === 'dark'
        ? themeQuartz.withPart(colorSchemeDarkBlue)
        : themeQuartz.withPart(colorSchemeLightCold)
        " @grid-ready="onGridReady" :cellSelection="cellSelection" :rowSelection="rowSelection"
      :rowGroupPanelShow="rowGroupPanelShow" :aggFuncs="aggFuncs" :getRowId="getRowId"
      :getContextMenuItems="getContextMenuItems" :treeData="false" :onColumnRowGroupChanged="onColumnRowGroupChanged"
      :undoRedoCellEditing="true" :undoRedoCellEditingLimit="10" @cell-value-changed="cell_value_changed">
    </ag-grid-vue>
...
const getContextMenuItems = (params: GetContextMenuItemsParams) => {
  let data = props.table_configuration_func(`${props.table_name}`);

  let conf = data.ToolBarConf;
  console.log(conf);
  const delete_option = {
    // custom item
...
async function loadTableFromBackend() {
  try {
    ...
    const get_url = props.get_url
      ? props.get_url
      : `/${props.app_name}/${props.table_name.replaceAll("#", "/")}/get`;

    // Fetch data from the backend
    const res = await useAPIFetch(get_url, { method: "get" });
        if (res?.error == null) {
          let tbl_data = res.data;
...
async function saveData() {
  ...
  //const res = await useAPIFetch(save_url, { method: "post", body: data });
    ...
    const rawData = data.map(proxy => toRaw(proxy))
      console.log(rawData);
    useAPIFetch(save_url, { method: "post", body: rawData })
        .then(async (res: any) => {
          if (res.error) {
...
const cell_value_changed = (event: any) => {
  if (event?.api) {
