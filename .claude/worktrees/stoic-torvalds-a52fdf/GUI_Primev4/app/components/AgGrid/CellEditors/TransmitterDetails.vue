...
<template>
  <Card>
    <template #title> Transmitter Details</template>
    <template #content>

  <div class="grid mt-1">
    <div class="col">
      
        <h6 class="mb-0">Transmitter Name</h6>
        <InputText id="transmitter_name" placeholder="Example: C Transmitter 1" v-model="transmitter_name" type="text" :class="{ 'p-invalid': errorMessage }"
          aria-describedby="text-error" />
        <div><span class="p-error" id="text-error">{{ errorMessage || '&nbsp;' }}</span></div>
          
    </div>
    <div class="col">
      <h6 class="mb-0">Transmitter Code</h6>
        <InputText id="transmitter_code" placeholder="Example: CTX1" v-model="transmitter_code" type="text" :class="{ 'p-invalid': errorMessage }"
          aria-describedby="text-error" />
        <div><span class="p-error" id="text-error">{{ errorMessage || '&nbsp;' }}</span></div>
    </div>
</div>
...
<div class="grid">
  <div class="col">
      <h6 class="mb-0">Down Link Ports</h6>
    ...
    <div class="col">
      <h6 class="mb-0">Select Modulation Type</h6>
           <Dropdown v-model="selectedModulation" :options="downLinkModulationTypes" filter inputId="select_modulation_type" optionLabel="value" placeholder=" Select Modulation Type"
           class="w-full">
            </Dropdown>
     </div>
</div> 
      ...
      <span class="p-buttonset">
        <Button @click="ValueChanged" label="Save" icon="pi pi-check" />
        <Button @click="ValueChanged" label="Cancel" icon="pi pi-times" />
      </span>
    </template>
  </Card>
