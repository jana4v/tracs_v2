...
// MnemonicLoader loads mnemonics from the TMMnemonicStore where enable_limit=true.
// It provides thread-safe access for the monitor loop and supports
...
// Reload reloads mnemonics. Called when TM_LIMIT_CHANGED is received.
...
// loadFromStore fetches all mnemonics and filters to those with enable_limit=true.
func (l *MnemonicLoader) loadFromStore(ctx context.Context) error {
	all, err := l.store.FindAll(ctx)
	if err != nil {
		return fmt.Errorf("find mnemonics with enable_limit=true: %w", err)
	}
	...
	l.logger.Info("loaded mnemonics", "count", len(results), "filter", "enable_limit=true")
	return nil
}
