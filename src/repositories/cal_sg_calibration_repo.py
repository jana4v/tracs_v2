...
class CalSgCalibrationRepository:
    def __init__(self, db_path: str, table_name: str = "CalSGCalibrationData") -> None:
    ...
    def _migrate_schema_if_needed(self) -> None:
        ...
        has_cal_id = "CalId" in column_names
        composite_pk = {str(row[1]) for row in info_rows if int(row[5] or 0) > 0}

        if has_cal_id and composite_pk == {"CalId", "Frequency"}:
            return
        ...
        if has_cal_id:
            self._conn.execute(
                f"""
                INSERT OR REPLACE INTO {temp_table} (CalId, Frequency, Value, DateTime)
                SELECT COALESCE(CalId, ''), Frequency, Value, DateTime FROM {self._table_name}
                """
            )
        else:
            self._conn.execute(
                f"""
                INSERT OR REPLACE INTO {temp_table} (CalId, Frequency, Value, DateTime)
                SELECT '', Frequency, Value, DateTime FROM {self._table_name}
                """
            )
    ...
    def upsert(self, cal_id: str, frequency: float, value: float, date_time: datetime) -> None:
        with self._lock:
            self._conn.execute(
                f"""
                INSERT INTO {self._table_name} (CalId, Frequency, Value, DateTime)
                VALUES (?, ?, ?, ?)
        ...
        with self._lock:
            if cal_id is None:
                cursor = self._conn.execute(
                    f"SELECT CalId, Frequency, Value, DateTime FROM {self._table_name} ORDER BY Frequency"
                )
            else:
                cursor = self._conn.execute(
                    f"SELECT CalId, Frequency, Value, DateTime FROM {self._table_name} WHERE CalId = ? ORDER BY Frequency",
                    (str(cal_id or "").strip(),),
                )
