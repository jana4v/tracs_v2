...
class AG_N1914A(PowerMeter):
    ...
    def preset_instrument(self):
        ...
        for channel_number in [1, 2]:
            code += (
                f':SENS{channel_number}:LIM:STAT ON;'
                f':SENS{channel_number}:LIM:UPP:DATA {upper_limit} DBM;'
                f':SENS{channel_number}:LIM:LOW:DATA {lower_limit} DBM;'
                f':CALC{channel_number}:GAIN 0;'
            )
        self.command(code)
        return self
...
factory.register_component(InstrumentTypes.PowerMeter,"AG_N1914A",AG_N1914A)