...
logger = logging.getLogger("BHE_KU")
...
class BHE_KU(TtcTransmitter):
    ...
    def initialize_specifications(self) -> Specification:
        ...
        specification.frequency = Limits(lower_limit=12500, upper_limit=14000, units=Units.MHz)
    ...
    def set_power_level(self, power):
        return self.set_carrierLevel(power)

    def set_carrierLevel(self, power):
    ...
    def set_frequency(self, frequency):
        ...
        code = '*FREQ=' + self.decode_to_hex(frequency, 4)
        result = self.command(code, read_operation=True, buffer_length=100)
        return self

    def set_rf_on(self, onoff=1):
        result = self.command('*RFON=01\r', read_operation=True, buffer_length=100)
        return self
    ...
    def set_pm_mod_index(self, mod_index):
        ...
        mod_index = math.trunc(mod_index * 20)
        code = '*PMOD=' + str(mod_index) + '\r'
        result = self.command(code, read_operation=True, buffer_length=100)
        self._statusupdate(result)
        return self
...
factory.register_component(InstrumentTypes.TtcTransmitter, "BHE_KU", BHE_KU)
